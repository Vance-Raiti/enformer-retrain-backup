...
class MeanPearsonCorrCoefPerChannel(Metric):
    ...
    def __init__(self, n_channels:int, dist_sync_on_step=False,**kwargs):
...
class poisson_loss(torch.nn.PoissonNLLLoss):
    def __init__(self,**kwargs):
        super().__init__(log_input=False)
    def forward(self,preds,targets):
        return super().forward(preds,targets).mean()
...
class mse_loss(torch.nn.Module):
    ...
    def forward(self,preds,targets):
        return self.loss(preds,targets).mean()
Metrics = {
    'name': 'Metrics',
    'poisson_loss': poisson_loss,
    'mean_corr_coef': MeanPearsonCorrCoefPerChannel,
    'mse_loss': mse_loss
}
